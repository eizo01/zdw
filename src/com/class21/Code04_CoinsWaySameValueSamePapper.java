 ...
 * 一共就3种方法，所以返回3
 * 思路 去重，info结构，货币面值是一个数组，张数是一个数组
 *
 */
public class Code04_CoinsWaySameValueSamePapper {
    ...
    public static Info getInfo(int[] arr) {
        ...
        return new Info(conins, zhangs);
    }// coins 面值数组，正数且去重

    // zhangs 每种面值对应的张数
    public static int process(int[] coins, int[] zhangs, int index, int rest){
        //如果 走到最后一张纸币， 凑到返回1种方法
        if (index == coins.length){
            return rest == 0 ? 1 : 0;
        }
        int ways = 0;
        for (int zhang = 0; zhang <=zhangs[index] && zhang * coins[index] <= rest; zhang ++){
            ways += process(coins,zhangs,index + 1, rest - zhang * coins[zhang]);
        }
        return ways;
    }
    ...
    public static int dp2(int[] arr, int aim) {
        ...
        for (int index = N - 1; index >= 0; index--) {
            for (int rest = 0; rest <= aim; rest++) {
                ...
                // rest 减去一张纸币 小于，说明越界了
                if (rest - coins[index] >= 0){
                    // 获得我左边的星号
                    dp[index][rest] += dp[index - 1][rest] + dp[index][rest - coins[index]];
                }
                // 加一 位置 才是 甲位置 如果rest - 这个位置的钱不是负数，所以才有处理星号多加这个位置
